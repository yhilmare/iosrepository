...
@interface YHResumeBasicViewController ()<UIScrollViewDelegate, UIPageViewControllerDataSource, UIPageViewControllerDelegate>
...
@implementation YHResumeBasicViewController
        ...
        for (YHResume *res in result.rows) {
            ...
            [YHResumeTool getResumeBasicInfo:^(YHResultItem *result) {
                for (YHResumeBasicInfo *res in result.rows) {
                    ...
                    one.resumeIntergrity = res.SelfEvaluation;
                    ...
                    if([res.ResumeId isEqualToString:self.resumeId]){
